...
\section{Worker Assignment Algorithm Implementation}
\label{app:worker-assignment}
...
def assign_workers(nodes, predict, base_rc, SLA, MAX_CLUSTERING):
    assigned = set()
    ...
    for n in nodes:                
        ...
        if not n.upstream: # root nodes
            ...
            outputs = {u.worker: predict.output_size(u) for u in n.upstream}
            best = max(outputs, key=outputs.get)
            assign_worker([n], best)
    
    def assign_group(up_worker, tasks):
        if not tasks: return
        exec_t = {t: predict.exec_time(t) for t in tasks}
        out_sz = {t: predict.output_size(t) for t in tasks}
        ...
        while longs:
            ...
            longs = longs[half:]
\end{lstlisting}
\caption{Worker ID Assignment Algorithm Implementation}
