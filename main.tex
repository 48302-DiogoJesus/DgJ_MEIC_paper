...
\begin{figure*}[h]
  \centering
  \includegraphics[width=\textwidth]{figures/solution_distributedarchintegrated.png}
...
\begin{figure*}[h]
    \centering
    \includegraphics[width=\textwidth]{figures/eval/1_predictions/prediction_accuracy_by_sla_and_fullfilment.png}
...
\begin{figure*}[h]
    \centering
    \includegraphics[width=\textwidth]{figures/eval/3_opts/time breakdown.png}
    \caption{Time breakdown analysis showing the median time spent on worker startup, waiting for inputs, execution, and uploading results.}
    \label{fig:time_breakdown_analysis}
\end{figure*}

\begin{figure*}[h]
    \centering
    \includegraphics[width=\textwidth]{figures/eval/4_res_usage/us vs wukong makespan res usage.png}
    \caption{\textit{Makespan} vs Resource Usage comparison, showing the trade-off between execution time and resource consumption (lower is better for both).}
    \label{fig:makespan_resource_usage_tradeoff}
\end{figure*}

\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth]{figures/eval/2_metrics/makespan dist.png}
    \caption{\textit{Makespan} distribution across all planners. Lower values indicate better performance.}
    \label{fig:makespan_dist}
\end{figure}

Our evaluation focused on \textbf{prediction accuracy}, \textbf{\textit{makespan}}, and \textbf{resource efficiency} (measured in GB-seconds, based on AWS Lambda's cost calculation~\footnote{\url{https://aws.amazon.com/lambda/pricing/}}). As shown in Figure~\ref{fig:predictions_accuracy_by_sla_and_fullfilment}, our system's predictions for task execution time proved highly accurate (median relative error below 9.3\%), while data transfer time predictions were less precise (around 35\% error), highlighting I/O variability even in a controlled and semi-predictable environment.


Figure~\ref{fig:time_breakdown_analysis} breaks down the median workflow execution time into its primary components. This analysis reveals that the \textbf{WUKONG} planner spends a significantly larger portion of its time on worker startup compared to our planners. This is a direct result of its scheduling strategy, which launches far more workers (a median of 32 vs. our planners' 13) for workflows with fan-outs. The chart also visually confirms the effectiveness of our optimizations, which reduce the time spent waiting for dependencies.

Figure~\ref{fig:makespan_dist} compares the total workflow completion time (makespan) across all planners. All variations of our planners outperform the WUKONG implementations. The optimized \textbf{Non-Uniform} planner consistently achieves the lowest makespan, demonstrating a clear performance benefit from both the adaptive resource allocation and the optimizations.

Finally, Figure~\ref{fig:makespan_resource_usage_tradeoff} synthesizes these results by plotting makespan against resource usage, illustrating the critical trade-off between speed and cost-efficiency. The optimized \textbf{Non-Uniform} planner is the fastest, achieving a median makespan of 4.31s—a 53\% reduction compared to its non-optimized version. However, the optimized \textbf{Uniform} planner is the most resource-efficient, consuming the least GB-seconds. This highlights a key choice for users: the optimized Non-Uniform planner is 60\% faster than the optimized Uniform one but requires 54\% more memory resources to achieve that speed.

\subsection{Analysis}
The results demonstrate that our proposed planners achieve a balanced trade-off between performance and efficiency, with our optimizations significantly enhancing both aspects. The \textbf{Non-Uniform} planner consistently delivers the shortest makespan, while the \textbf{Uniform} planner remains the most resource-efficient, achieving the lowest median GB-seconds usage.

The WUKONG scheduling strategy, with its policy of invoking a new worker for almost every task in a fan-out, suffers from high worker startup overhead and synchronization delays. This leads to higher resource consumption and makes it less suitable for the workflows with large fan-outs tested in our evaluation.

Our optimizations, \textit{pre-loading} and \textit{pre-warming}, proved highly effective. They not only reduced makespan by 3 to 5 seconds in some cases but also increased the rate of warm starts by an average of 25\%, directly contributing to both performance and cost-effectiveness. Resource contention, however, remained a challenge for the \textbf{Uniform} planner, which can execute multiple tasks in parallel on less powerful workers. The \textbf{Non-Uniform} planner's use of more powerful workers helped mitigate these effects. Overall, the results validate our design goals of improving execution efficiency through predictive planning and intelligent task co-location.
...
This work explored prediction-based scheduling for serverless workflows, aiming to reduce makespan and improve resource efficiency by leveraging historical task metrics. We designed, implemented, and evaluated a decentralized workflow engine that integrates predictive planning with global knowledge of workflow structure.

Our evaluation demonstrated that predictive scheduling provides tangible benefits. Across multiple workflows, our planners generally outperformed WUKONG, a state-of-the-art decentralized engine. The optimized \textbf{Non-Uniform} planner achieved the lowest makespan, while the \textbf{Uniform} planner proved to be more resource-efficient. These improvements stem from co-location strategies and optimizations such as \textbf{pre-warming} and \textbf{pre-loading}, which reduced both worker startup delays and data waiting times.

Beyond performance results, this work contributes a modular framework separating \textbf{prediction}, \textbf{planning}, and \textbf{execution} layers, providing a foundation for future research in predictive scheduling.


\subsection{Limitations and Future Work}

The current solution has limitations that suggest directions for further work. Historical data management could be improved with summarization or downsampling to maintain scalability. Optimization handling could benefit from stronger abstractions to manage conflicting strategies, and more robust error handling would improve reliability.

Future work could explore more aggressive optimizations, such as \textit{task duplication}, which trades higher resource usage for lower latency, and support for dynamic workflow structures, allowing fan-outs to vary at runtime. Enhancing user control—through minimum resource specifications for tasks and interactive dashboards—would increase usability and observability.

Finally, broader evaluation is needed to assess generalizability. Comparative studies of alternative prediction strategies, deployment on commercial FaaS platforms, and tests with complex real-world workflows would provide a more complete understanding of the system's performance, strengths, and limitations.
