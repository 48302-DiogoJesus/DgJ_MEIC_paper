...
% TODO: introductory section text
...
In addition, there are also open-source workflow orchestration projects that run on general-purpose cloud or on-premises infrastructure, combatting vendor lock-in by providing better interoperability and flexibility. Apache Airflow~\cite{apache_airflow} is a widely used example, allowing workflows to easily be defined in Python code. It provides fine-grained scheduling, monitoring, and retry policies, being mostly used for ETL ~\footnote{\label{fn:etl}https://pt.wikipedia.org/wiki/Extract,\_transform,\_load} processes, machine learning workflows, and batch data processing. Another open-source alternative is Luigi~\footnote{\label{fn:luigi}https://github.com/spotify/luigi}, also a Python-based workflow framework, developed and used at Spotify for managing complex batch data pipelines.
