...
This section summarizes the main evaluation results of all proposed planners and their optimized versions, focusing on prediction accuracy, workflow execution time (\textit{makespan}), and resource efficiency.
...
Figure~\ref{fig:predictions_accuracy_by_sla_and_fullfilment} shows the prediction accuracy and SLA fulfillment rates across all planners (excluding WUKONG). Execution time predictions were highly accurate, with median relative errors below \textbf{9.3\%}, while data transfer times (upload/download) were less precise, around \textbf{35\%}. SLA fulfillment rates aligned with expectations: \textbf{41.9-71.5\%} for P50, \textbf{66.7-89.7\%} for P75, and \textbf{86.9-100\%} for P90. These results confirm the reliability of the predictive models for workflow planning and SLA adherence.
...
The makespan distribution in Figure~\ref{fig:makespan_dist} shows that all versions of our planners outperformed both WUKONG implementations. The non-optimized \textbf{Uniform} planner was \textbf{13.4\% faster} than the optimized WUKONG, while the optimized \textbf{Non-Uniform} planner achieved the best overall makespan. Optimization improved execution speed by up to \textbf{53\%} in the Non-Uniform case.
...
Figure~\ref{fig:time_breakdown_analysis} shows that our planners significantly reduced worker startup and dependency waiting times compared to WUKONG. Optimized versions benefited from both \textit{pre-loading} and \textit{pre-warming}, reducing startup overhead and saving between \textbf{3-5 seconds}. The optimized \textbf{Non-Uniform} planner achieved the shortest execution times, while the optimized \textbf{Uniform} planner remained the most resource-efficient.
...
Figures~\ref{fig:makespan_comparison} and~\ref{fig:resources_comparison} summarize the trade-off between execution time and resource usage, respectively. The optimized \textbf{Non-Uniform} planner achieved a \textbf{53\% faster makespan} while consuming \textbf{45\% less memory} than its base version, emerging as the best-performing planner overall. Conversely, the optimized \textbf{Uniform} planner achieved the lowest resource usage, consuming only \textbf{174.5 GB-seconds} (\textbf{74\% less} than the Non-Uniform version) while remaining within \textbf{9.6\%} of its execution speed.

In contrast, the \textbf{WUKONG} planner remained slower and more resource-intensive: it was \textbf{17.5\% slower} and consumed \textbf{108\% more memory} than the Uniform planner. Its optimized version offered only modest gains (\textbf{2.7\%} faster, \textbf{25\%} less memory), confirming the superiority of our approaches.

\subsection{Summary}

Overall, the results demonstrate that our planners achieve a strong balance between speed and efficiency. The \textbf{Non-Uniform} planner excels in performance for the tested workflows, while the \textbf{Uniform} planner is the most cost-efficient, performing better than \textbf{WUKONG} while also consuming less resources. Both benefit substantially from the proposed \textit{pre-loading} and \textit{pre-warming} optimizations, validating their design for efficient, serverless workflow execution.
